...
# ── Colors ────────────────────────────────────────────────────────────────────
# Light "off-white private-bank" palette.
# Variable names are kept the same so the rest of the file is untouched;
# only the values changed (dark navy theme → warm off-white theme).
NAVY   = "#EDE8DF"   # soft sand — secondary surface (inputs, info boxes, table header/alt rows)
SLATE  = "#E2DDD2"   # hairline borders / chart gridlines
SILVER = "#8A8475"   # muted warm-grey — labels & secondary text
WHITE  = "#1B1E22"   # near-black — strong headings, values, key text
GREEN  = "#1A7F4F"   # positive
RED    = "#C0392B"   # negative
GOLD   = "#9A7B33"   # deep gold accent (section headers, borders, badges)
BG     = "#F4F1EA"   # warm off-white page background
CARD   = "#FFFFFF"   # white panels / cards
BLUE   = "#2E6CA8"   # info / Fed accent
TEXT   = "#3B3F45"   # body text
...
with st.sidebar:
    ...
    today = date.today()
    # Keys load from Render environment variables OR Streamlit Cloud secrets.
    # The try/except prevents a crash on hosts that have no secrets.toml file.
    ...
    fred_key      = _load_key("FRED_API_KEY")
...
">
    <div style="font-size:11px;font-weight:700;color:#9A7B33;letter-spacing:0.06em;line-height:1.5">Built by Emmitt Macor</div>
</div>""", unsafe_allow_html=True)
...
if ai_events:
    ...
    CAT_COLORS = {
        ...
        "GDP": GOLD, "Economic Data": SILVER, "Earnings": "#7A57C0",
    }
