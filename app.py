...
.sbox li {{ margin-bottom: 10px; color: {TEXT}; line-height: 1.65; font-size: 13px; }}
...
def get_prices(ticker: str) -> pd.DataFrame:
    try:
        df = yf.download(ticker, period="3mo", progress=False, auto_adjust=True, threads=True)
        if df.empty:
            return pd.DataFrame()
...
def price_stats(df: pd.DataFrame, week_end: date) -> dict:
    ...
    return {
        ...
        "high_52":   round(float(df["High"].tail(65).max()), 2),
        "low_52":    round(float(df["Low"].tail(65).min()), 2),
...
">
    ...
    <div style="font-size:9px;color:#4A5A6A;letter-spacing:0.06em">The Goat</div>
</div>""", unsafe_allow_html=True)
...
with st.spinner("Loading market data…"):
    ...
    # Download all tickers in parallel (18 tickers × ~0.3s = 0.5s total vs 5s sequential)
    all_tickers = {**INDEX_TICKERS, **SECTOR_TICKERS}
    all_dfs = {}
    with ThreadPoolExecutor(max_workers=8) as ex:
        futures = {ex.submit(get_prices, ticker): name for name, ticker in all_tickers.items()}
        for future in as_completed(futures):
            name = futures[future]
            try:
                all_dfs[name] = future.result()
            except Exception:
                all_dfs[name] = pd.DataFrame()

    idx_dfs   = {n: all_dfs.get(n, pd.DataFrame()) for n in INDEX_TICKERS}
    idx_stats = {n: price_stats(df, week_end) for n, df in idx_dfs.items() if not df.empty}
    sec_dfs   = {n: all_dfs.get(n, pd.DataFrame()) for n in SECTOR_TICKERS}
    sec_stats = {n: price_stats(df, week_end) for n, df in sec_dfs.items() if not df.empty}

    # FRED and news load in parallel too
    fred = {}
    if fred_key:
        with ThreadPoolExecutor(max_workers=6) as ex:
            fred_futures = {ex.submit(get_fred, sid, fred_key): label
                           for label, (sid, _, _) in FRED_SERIES.items()}
            for future in as_completed(fred_futures):
                label = fred_futures[future]
                try:
                    fred[label] = future.result()
                except Exception:
                    fred[label] = pd.DataFrame()
...
Write exactly 6 bullet points in HTML <li> tags. Format rules:
- Start each with <b>Section Title:</b>
- MAX 2 sentences per bullet — be tight and punchy, no rambling
- Lead with the key fact, then the implication. No filler.
- Use em-dashes (—) to connect cause → effect efficiently
- Numbers and % where possible. Name specific indexes, sectors, stocks.
- Think Bloomberg Terminal brief, not research report

1. <b>Market Overview:</b> One sentence on direction + magnitude. One sentence on the dominant narrative.
2. <b>Key Drivers:</b> List exactly 3 catalysts as: "· [Event] → [market impact]" on separate lines
3. <b>Index Performance:</b> Best and worst index with %, one sentence explaining the divergence.
4. <b>Sector Rotation:</b> Top and bottom sector with %, one sentence on what the rotation signals.
5. <b>Volatility:</b> VIX level + weekly move, one sentence on what it implies for positioning.
6. <b>What to Watch:</b> Three upcoming events as "· [Event] ([date]) — [why it matters]" on separate lines

Return ONLY the 6 <li> tags. Use <br> for line breaks within a bullet."""
...
@st.cache_data(ttl=21600, show_spinner=False)
def generate_daily_summary(today_str: str, api_key: str, snapshot: str) -> str:
    """AI-generated today's market changes with drivers."""
    if not api_key:
        return ""
    prompt = f"""You are a senior market strategist. Today is {today_str}.

Live market data:
{snapshot}

Search the web for today's market news then write exactly 3 bullet points in HTML <li> tags.

Format rules — STRICT:
- MAX 2 lines per bullet. Short. Punchy. Bloomberg-style.
- Lead with the key number/fact, then the reason
- Use "·" for sub-points within a bullet, separated by <br>

1. <b>Today's Move:</b> S&P/Nasdaq/Dow direction + % today. One line on which indexes led vs lagged.
2. <b>Today's Drivers:</b> Exactly 3 catalysts as:<br>· [Catalyst] → [effect on market]<br>· [Catalyst] → [effect]<br>· [Catalyst] → [effect]
3. <b>Market Tone:</b> Risk-on or risk-off? One sentence. Then: "Watching: [1-2 key levels or events traders are focused on today]"

Use web search. Be specific to {today_str}. Real names, real numbers.
Return ONLY the 3 <li> tags. Use <br> for line breaks."""

    result = ai_generate(prompt, f"daily_summary_{today_str}", api_key)
    if result:
        import re as _re
        result = _re.sub(r'```[\w]*\n?', '', result).strip()
        result = _re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', result)
    return result if result and "<li>" in result else ""


# Build and render the summary — two sections: Today + This Week
week_str   = week_end.strftime("%B %d, %Y")
today_str  = date.today().strftime("%B %d, %Y")
snapshot   = build_market_snapshot()
static_html = _static_summary()

# Loading placeholder
summary_placeholder = st.empty()
summary_placeholder.markdown(
    f'<div style="background:{CARD};border:1px solid {SLATE};border-radius:8px;'
    f'padding:16px 20px;min-height:80px">'
    f'<div style="color:{SILVER};font-size:12px;font-style:italic">Generating market analysis…</div>'
    f'</div>',
    unsafe_allow_html=True
)
...
with st.spinner(""):
    ...
    with _TPE(max_workers=2) as _ex:
        _daily_fut  = _ex.submit(generate_daily_summary, today_str, anthropic_key, snapshot)
        _weekly_fut = _ex.submit(generate_rich_summary)
        daily_html  = _daily_fut.result()
        weekly_html = _weekly_fut.result()

# Clean weekly html
import re as _re
if weekly_html:
    weekly_html = _re.sub(r'```[\w]*\n?', '', weekly_html).strip()
    weekly_html = _re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', weekly_html)
    if "What to Watch" not in weekly_html:
        ev = upcoming_events()[:3]
        watch_str = "; ".join(f"{e['event']} ({e['date']})" for e in ev) if ev else "CPI, PCE, jobs report, and Fed communications"
        weekly_html += f"<li style='margin-bottom:10px'><b>What to Watch Next Week:</b> Key catalysts include: {watch_str}. Any surprise could meaningfully reprice rate expectations and equity direction.</li>"

# Render two-section summary
daily_content  = daily_html  if daily_html  else ""
weekly_content = weekly_html if weekly_html else static_html

# Section label style
def _sec_label(text, color):
    return (f'<div style="font-size:9px;font-weight:700;letter-spacing:0.12em;'
            f'text-transform:uppercase;color:{color};margin-bottom:10px;'
            f'padding-bottom:6px;border-bottom:1px solid {SLATE}">{text}</div>')

daily_block = (
    _sec_label(f"📊 Today's Market  ·  {today_str}", BLUE)
    + f'<ul style="margin:0;padding-left:18px">{daily_content}</ul>'
) if daily_content else (
    _sec_label(f"📊 Today's Market  ·  {today_str}", SILVER)
    + f'<div style="color:{SILVER};font-size:12px;font-style:italic">Daily summary unavailable — check back shortly.</div>'
)

weekly_block = (
    _sec_label(f"📅 Week of {week_str}", GOLD)
    + f'<ul style="margin:0;padding-left:18px">{weekly_content}</ul>'
)

summary_placeholder.markdown(
    f'<div style="background:{CARD};border:1px solid {SLATE};border-radius:8px;padding:16px 20px">'
    f'{daily_block}'
    f'<div style="margin:16px 0 14px;border-top:1px solid {SLATE}"></div>'
    f'{weekly_block}'
    f'</div>',
    unsafe_allow_html=True
)
